...
export function ReviewsPage() {
  ...
  const renderReviewsPager = (className?: string) => (
    <ListPager
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-x-hidden md:h-full md:overflow-hidden">
      ...
      <div className="flex min-h-0 flex-1 flex-col px-4 py-4 sm:px-6 sm:py-6 md:overflow-hidden">
        ...
        <ResponsiveRecordLayout
          ...
          renderPager={renderReviewsPager}
          mobileCards={
            <MobileRecordList>
              {pageItems.map((row) => {
                ...
                return (
                  ...
                  >
                    <MobileRecordTop
                      select={
                        <MobileSelect
                          ...
                          label={`Select review by ${row.memberName}`}
                        />
                      }
                      title={
                        <button
                      ...
                      subtitle={row.merchantName}
                      badge={<ReviewStatusBadge status={row.status} />}
                      action={
                        <RowActionButton
                    ...
                    />
                    <MobileRecordFields>
                      <MobileRecordField label="Rating" value={stars(row.rating)} />
                      <MobileRecordField label="Date" value={formatDate(row.submittedAt)} />
                      <MobileRecordField
                        label="Review"
                        value={<span className="whitespace-pre-wrap break-words">{row.text}</span>}
                        wide
                      />
                    </MobileRecordFields>
                  </MobileRecordCard>
